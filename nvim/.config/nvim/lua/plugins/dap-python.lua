...
return {
  ...
  config = function()
    ...
    dap.configurations.python = {
      ...
      {
        -- Launch FastAPI server with debugpy
        -- Equivalent to: python -m debugpy --listen 5678 --wait-for-client -m uvicorn app.main:app --reload
        name = "Python: Launch FastAPI server",
        type = "python",
        request = "launch",
        module = "debugpy",
        args = { "--listen", "5678", "--wait-for-client", "-m", "uvicorn", "app.main:app", "--reload" },
        console = "integratedTerminal",
        cwd = "${workspaceFolder}",
        env = {
